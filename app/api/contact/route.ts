...
export async function POST(req: NextRequest) {
  try {
    const { name, surname, email, product, message, openedAt } = await req.json();

    if (!name || !surname || !email) {
      return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
    }

    // Anti-bot: invio in meno di 3 secondi = bot (l'honeypot dava falsi
    // positivi a causa dell'autofill del browser, quindi non lo usiamo più)
